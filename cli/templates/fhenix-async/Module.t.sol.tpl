...
contract {{MODULE_NAME}}Test is Test {
    ...
    function testRequestsFheDuringBeforeSwap() public {
        ...
        assertEq(manager.lastTaskId(), 1);
    }

    function testFulfillmentRoutesToHandler() public {
        bytes memory payload = abi.encode("hook-data");
        vm.prank(AGGREGATOR);
        module.beforeSwap(IPoolManager(address(0)), address(this), key, params, payload);

        vm.expectEmit(true, true, true, true);
        emit {{MODULE_NAME}}.FheResultHandled(1, address(this), {{MODULE_NAME}}.HOOK_BEFORE_SWAP(), abi.encode("result"));
        module.onFheResult(1, abi.encode("result"));
    }
...
contract MockFheManager is IFheComputationManager {
