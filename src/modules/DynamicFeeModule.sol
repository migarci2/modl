...
contract DynamicFeeModule is IMODLModule, Ownable {
    ...
    event VolatilityUpdated(uint24 index, uint24 multiplier);
    ...
    constructor(address aggregator_, uint24 minFee_, uint24 maxFee_, uint24 baseFee_, uint24 volatilityMultiplier_)
        Ownable(msg.sender)
    {
    ...
    function setVolatilityIndex(uint24 newIndex) external onlyOwner {
        volatilityIndex = newIndex;
        emit VolatilityUpdated(newIndex, volatilityMultiplier);
    }

    function setVolatilityMultiplier(uint24 multiplier) external onlyOwner {
        volatilityMultiplier = multiplier;
        emit VolatilityUpdated(volatilityIndex, multiplier);
    }
